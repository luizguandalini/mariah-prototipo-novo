...
export default function GaleriaImagens() {
  ...
  // O modal agora vive em `EditImagemModal` (compartilhado com o
  // preview de PDF). A galeria só precisa saber QUAL imagem está
  // aberta; estado de caption/marker/erros/scroll-lock é interno
  // ao modal.
  ...
  // `LEGENDA_MAX` e `LEGENDA_DEBOUNCE_MS` ficaram dentro do componente
  // compartilhado `EditImagemModal` — a galeria não gerencia mais o
  // textarea da legenda.
  ...
  const openLightbox = useCallback(
    (index: number) => {
      ...
      // Para fotos AVARIA sem marker salvo, abre o lightbox já em
      // modo edição — o placeholder centralizado do `EditImagemModal`
      // aparece e o usuário arrasta para posicionar.
  ...
  const closeLightbox = useCallback(() => {
    ...
    // O flush da legenda é responsabilidade do `EditImagemModal`
    // (que dispara `onLegendaFlush` antes de chamar `onClose`).
    setLightboxOpen(false);
    setLightboxIndex(null);
    setEditingImageId(null);
  }, []);

  // Persistência simples de legenda chamada pelo `EditImagemModal`.
  // Atualiza otimisticamente a galeria e dispara o PATCH. Sem
  // debounce aqui — o modal já tem seu próprio debounce interno.
  const handleLegendaChangeApi = useCallback(
  ...
  const goToLightboxImage = useCallback(
    (newIndex: number) => {
      ...
      // nova começa em modo leitura (com ou sem marker). O
      // `EditImagemModal` recria o estado interno (caption,
      // loading, erros) a partir do `imagem` recebido via prop.
      setEditingImageId(null);
    },
  ...
  return (
    <DashboardLayout>
      ...
      >
        ...
        {/* ========== LIGHTBOX (visualização expandida) — agora compartilhado */}
        {lightboxOpen && lightboxIndex !== null && imagens[lightboxIndex] && (
          <EditImagemModal
            open={lightboxOpen}
            imagem={imagens[lightboxIndex]}
            onClose={closeLightbox}
            onMarkerChange={handleMarkerChange}
            onLegendaChange={handleLegendaChangeApi}
            onLegendaFlush={handleLegendaChangeApi}
            onPrev={
              lightboxIndex > 0 ? goToPreviousLightboxImage : undefined
            }
            onNext={
              lightboxIndex < imagens.length - 1
                ? goToNextLightboxImage
                : undefined
            }
            hasPrev={lightboxIndex > 0}
            hasNext={lightboxIndex < imagens.length - 1}
          />
        )}
      </div>
    </DashboardLayout>
